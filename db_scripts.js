...
function updateFreeShippingFlag() {
...
function updateFlagsImage()
{
 ...
 document.addEventListener("ShoptetDOMPageContentLoaded", function () {
  ...
  if (
      ...
      function SetUsetrite($th, usetrite) {
          var $root =
              cat === "category" || cat === "article"
                  ? $th.closest(".p")
                  : $th.closest(".product");

          $root.find(".price-standard > span").remove();
          $root.find(".price-save").remove();
          $root.find(".price-standard").before(usetrite);
      }
      ...
      function updateDiscountFlags() {
          $(".flags-extra .flag-discount").each(function () {
              ...
              var price_standard = $root.find(".price-standard").text();
              var price_standard_number = price_standard
                  .replace(",", ".")
                  .replace(/[^0-9.]/g, "");

              var price = $root.find(".price-final strong").text();
              var price_number = price.replace(",", ".").replace(/[^0-9.]/g, "");
              ...
              usetreno = usetreno.split(/(?=(?:\d{3})+(?:\.|$))/g).join(" ");
              ...
              usetreno_flag = usetreno_flag.replace(".", ",");
              SetUsetrite($th, usetreno_flag);
          });
      }

      function updateFreeShippingFlag2() {
      ...
      function refreshCategoryListing() {
          updateDiscountFlags();
          updateFreeShippingFlag2();
      }
  ...
  
  {
      ...
      function updateCategoryProductTiles() {
          $("span.flag-custom2:contains('Český produkt')").each(function () {
              var flag_remove = $(this).closest(".product").find(".p-in-in");
              $(this)
                  .insertAfter(flag_remove)
